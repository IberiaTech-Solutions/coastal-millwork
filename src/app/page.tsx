...
const SECTORS_WITH_PROJECTS = SECTOR_LABELS.filter((s) => PROJECTS_GALLERY.some((p) => p.sector === s)).map((label) => ({ label, href: "/projects" }));

/** Sectors We Serve: sectors with projects + Faith (always show Faith) */
const SECTORS_FOR_DISPLAY =
  SECTORS_WITH_PROJECTS.some((s) => s.label === "Faith") ? SECTORS_WITH_PROJECTS : [...SECTORS_WITH_PROJECTS, { label: "Faith" as const, href: "/projects" }];
...
const SECTOR_PHOTOS: { label: string; href: string; src?: string; alt?: string }[] = SECTORS_FOR_DISPLAY.map(({ label, href }) => {
  const project = PROJECTS_GALLERY.find((p) => p.sector === label);
  if (project) return { label, href, src: projectImageUrl(project, project.coverImage), alt: project.title };
...
export default function Home() {
  return (
    <div>
      ...
      {/* Sectors We Serve – photo grid (one image per sector, modern) */}
      <AnimateSection as="section" className="border-b border-[var(--border)] bg-subtle px-4 py-16 sm:py-22" aria-label="Sectors we serve">
